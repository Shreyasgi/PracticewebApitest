...
namespace webApitest.Controllers
{
    ...
    public class AdminDashboardController : ControllerBase
    {
        ...
        public AdminDashboardController(AppDbContext context, ILogger<AdminDashboardController> logger, IDocumentVerificationDLL documentVerificationService)
        {
            _context = context;
            _logger = logger;
        ...
        private async Task CallDLLVerification(int documentId)
        {
            ...
            {
                var document = await _context.UserUploadedDocuments.FindAsync(documentId);
                var original = await _context.OriginalDocuments
                    .FirstOrDefaultAsync(o => o.UserId == document.UserId);

                if (document == null || original == null)
                {
                    _logger.LogWarning($"Document {documentId} or original reference missing.");
                    return;
                }

                // Call DLL with 6 arguments
                var result = await _documentVerificationService.VerifyDocumentsAsync(
                    document.ECPath,
                    original.OriginalECPath,
                    document.AadhaarPath,
                    original.OriginalAadhaarPath,
                    document.PANPath,
                    original.OriginalPANPath
                );
